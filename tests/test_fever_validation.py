...
def test_evidence_chain_retrieval():
    ...
    test_cases = [
        {
            ...
            "evidence": [
                [[1, 1, "Paris", 0]]  # Single evidence from one page
            ],
        ...
        },
        {
            ...
            "evidence": [
                [[1, 1, "Leonardo_da_Vinci", 0], [1, 2, "Leonardo_da_Vinci", 1], [1, 3, "Leonardo_da_Vinci", 2]]
            ],
        ...
        },
        {
            ...
            "evidence": [
                [[1, 1, "Renaissance", 0], [1, 2, "Italian_Renaissance", 1], [1, 3, "Northern_Renaissance", 0]]
            ],
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(f"   Complexity Level: {actual_complexity} (expected: {expected_complexity})")
        print(f"   Requires Composition: {evidence_chain.get('requires_composition', False)}")
        ...
        if evidence_chain.get("single_sentence_evidence"):
            print(f"   Single Evidence: {len(evidence_chain['single_sentence_evidence'])} sentences")
            
        if evidence_chain.get("multi_sentence_evidence"):
            print(f"   Multi-Sentence Groups: {len(evidence_chain['multi_sentence_evidence'])}")
        ...
        print(f"   {status} Validation: Type {'✓' if type_matches else '✗'}, Complexity {'✓' if complexity_matches else '✗'}")
...
def test_evidence_composition_evaluation():
    ...
    print(f"   Requires Composition: {composition_evaluation.get('requires_composition')}")
    print(f"   Composition Effectiveness: {composition_evaluation.get('composition_effectiveness', 0):.3f}")
    ...
    if "response_analysis" in composition_evaluation:
        print(f"   Mean Integration Score: {composition_evaluation.get('mean_integration_score', 0):.3f}")
...
def test_fever_adapter_integration():
    ...
    try:
        ...
        for i, evidence_set in enumerate(fever_sample.evidence_sets):
            print(f"      Set {i}: Type={evidence_set.evidence_type}, Composition={evidence_set.requires_composition}")
