...
class TestEntailmentCoherence:
    ...
    def test_simple_nli_model(self):
        ...
        assert (
            nli_model.predict("Paris is beautiful", "Paris is beautiful")
            == "entailment"
        )
        ...
        assert result in [
            "contradiction",
            "neutral",
        ]  # Simple model might classify as neutral
...
class TestSelfCheckGPTAdapter:
    ...
    def test_multi_sample_mode(self):
        ...
        assert (
            len(prop_set.propositions) == 3
        )  # Each answer as one proposition when not segmented
    ...
    def test_consistency_groups(self):
        ...
        adapter = SelfCheckGPTAdapter(
            consistency_mode="multi_sample", segment_responses=True
        )
