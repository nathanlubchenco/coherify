...
def test_selfcheckgpt_consistency_methods():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        matches_expectation = (
            (expected == "high" and avg_consistency >= 0.7) or
            (expected == "medium" and 0.4 <= avg_consistency < 0.8) or
            (expected == "low" and 0.2 <= avg_consistency < 0.6) or
            (expected == "very_low" and avg_consistency < 0.5)
        )
        
        status = "✅" if matches_expectation else "⚠️"
        print(f"  {status} Validation: {'PASS' if matches_expectation else 'NEEDS REVIEW'}")
...
def test_edge_cases():
    ...
    bertscore = SelfCheckGPTMetrics.check_consistency_bertscore(main_response, empty_responses)
    ...
    ngram_score = SelfCheckGPTMetrics.check_consistency_ngram(main_response, single_word_responses)
    ...
    bertscore_long = SelfCheckGPTMetrics.check_consistency_bertscore(main_response, long_responses)
