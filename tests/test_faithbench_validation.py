...
def test_difficulty_evaluation():
    ...
    for result in difficulty_results:
        print(f"   Sample {result['sample_id']}: {result['difficulty_score']:.3f} ({result['category']})")
    ...
    print(f"   {status} Difficulty ordering: {'Reasonable' if reasonable_ordering else 'Needs review'}")
...
def test_challenging_case_filtering():
    ...
    for level in challenge_levels:
        ...
        if level == "all":
            assert len(filtered_samples) == len(samples), "All samples should be included"
        elif level == "easy":
            assert len(filtered_samples) <= len(samples), "Should filter out harder samples"
        elif level == "hard":
            assert len(filtered_samples) <= len(samples), "Should filter out easier samples"
...
def test_challenge_statistics():
    ...
    assert total_categorized == stats['total_samples'], "Difficulty categorization should sum to total"
    
    total_method_based = stats['model_disagreement_cases'] + stats['annotation_based_cases']
    assert total_method_based == stats['total_samples'], "Method-based categorization should sum to total"
    ...
    for model in target_models:
        ...
        if challenging_for_model:
            ...
            for sample in challenging_for_model[:3]:  # Show first 3
                ...
                print(f"      Sample {sample['sample_id']}: Model performance = {model_performance}")
...
def test_integration_with_existing_adapter():
    ...
    try:
        ...
        config = FaithBenchConfig(enable_challenging_case_filtering=True, challenge_level="hard")
        adapter = FaithBenchAdapter(config)
