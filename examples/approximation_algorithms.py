...
from coherify.approximation import (
    ...
    SamplingBasedApproximator, ClusterBasedApproximator, 
    HierarchicalCoherenceApproximator, IncrementalCoherenceTracker,
...
def sampling_approximation_demo():
    ...
    for strategy_name, sampler in samplers.items():
        ...
        for sample_size in sample_sizes:
            ...
            print(f"  Sample {sample_size:2d}: Score {result.approximate_score:.3f}{ci_str}, "
    ...
    for sample_size, stats in evaluation['sample_size_results'].items():
        print(f"  Sample {sample_size}: Mean {stats['mean_score']:.3f} ±{stats['std_score']:.3f}, "
...
def clustering_approximation_demo():
    ...
    for method in clustering_methods:
        ...
        for n_clusters in cluster_counts:
            ...
            print(f"  {n_clusters:2d} clusters: Score {result.approximate_score:.3f}, "
    ...
    for level_info in summary["levels"]:
        print(f"  Level {level_info['level']}: {level_info['num_clusters']} clusters, "
...
def incremental_tracking_demo():
    ...
    for i, prop_text in enumerate(propositions_to_add):
        ...
        coherent_marker = "🟢" if update.new_score > 0.7 else "🟡" if update.new_score > 0.4 else "🔴"
        incremental_marker = "⚡" if update.incremental_computation else "🔄"
        
        print(f"  {i+1:2d}. {coherent_marker} {incremental_marker} Score: {update.old_score:.3f} → {update.new_score:.3f} "
    ...
    for phase_name, propositions in stream_phases:
        ...
        for prop_text in propositions:
            ...
            if proposition_count % 3 == 0:  # Show every 3rd update
                print(f"  {proposition_count:2d}. Window: {result['window_coherence']:.3f}, "
...
def scalability_comparison_demo():
    ...
    methods["Sampling"] = lambda ps: sampling_approx.approximate_coherence(ps, 30).approximate_score
    
    # Clustering method
    clustering_approx = ClusterBasedApproximator(SemanticCoherence())
    methods["Clustering"] = lambda ps: clustering_approx.approximate_coherence(ps, 15).approximate_score
        ...
        for method_name, method_func in methods.items():
            ...
            try:
                ...
                if method_name == "Exact":
                    ...
                    speedup = f"{exact_time/computation_time:.1f}x" if exact_time else "N/A"
                
                print(f"{method_name:<12} {size:<6} {score:<8.3f} {computation_time:<10.3f} {speedup:<8}")
                
            except Exception as e:
                print(f"{method_name:<12} {size:<6} {'ERROR':<8} {'N/A':<10} {'N/A':<8}")
...
def main():
    ...
    try:
        ...
        print("⚡ Coherify now provides scalable coherence computation for large datasets!")
