...
def temperature_variance_benchmark_demo():
    ...
    try:
        ...
        adapter = APIEnhancedQAAdapter(
            benchmark_name="VarianceAnalysis",
            config=api_config,
            provider=provider
        )
        ...
        api_generated_props = [
            p for p in prop_set.propositions 
        ...
        if len(api_generated_props) > 1:
            variance_prop_set = PropositionSet(
                propositions=api_generated_props,
                context=test_sample["question"]
            )
...
def comprehensive_benchmark_evaluation():
    ...
    try:
        ...
        # Create adapter
        adapter = APIEnhancedQAAdapter(
            benchmark_name="ComprehensiveQA",
            config=api_config,
            provider=get_provider()
        )
        ...
        print(f"🔬 Evaluating {len(qa_dataset)} samples with {len(coherence_measures)} measures")
        ...
        print(f"  Avg generations/sample: {api_stats['average_generations_per_sample']:.1f}")
...
def api_vs_local_comparison():
    ...
    try:
        ...
        print(f"  Content Expansion: {len(api_prop_set.propositions) - len(local_prop_set.propositions)} propositions")
        
        # Show value-added content
        api_generated_props = [
            p for p in api_prop_set.propositions
