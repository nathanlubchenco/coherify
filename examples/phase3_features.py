...
from coherify.measures import (
    ...
    ConfidenceBasedProbabilityEstimator, EnsembleProbabilityEstimator
)
...
def traditional_shogenji_demo():
    ...
    print(f"Individual Probabilities: {[f'{p:.3f}' for p in result1.details['individual_probabilities']]}")
    print(f"Interpretation: {result1.details['interpretation']}")
    
    print("\n2. Incoherent Propositions Example:")
    incoherent_answer = "Cats are animals. Ice cream is cold. The moon is made of cheese."
...
def probability_estimation_comparison():
    ...
    # Test proposition
    test_answer = "Paris is the capital of France. France is in Europe. Europe is a continent."
    ...
    for name, estimator in estimators.items():
        ...
        print(f"  Individual Probs: {[f'{p:.3f}' for p in result.details['individual_probabilities']]}")
...
def visualization_demo():
    ...
    measures = [
        ...
        HybridCoherence(semantic_weight=0.6, entailment_weight=0.4, nli_model=SimpleNLIModel())
    ]
    ...
    if 'component_scores' in hybrid_result.details:
        ...
        fig2 = visualizer.plot_component_analysis(
            hybrid_result,
            title="Hybrid Coherence Component Analysis"
        )
        plt.show(block=False)
    ...
    if hasattr(semantic_measure, 'encoder'):
        try:
            ...
            fig3 = visualizer.plot_similarity_matrix(
                ...
                title="Proposition Similarity Matrix"
            )
            plt.show(block=False)
...
def comprehensive_analysis_demo():
    ...
    prop_set = PropositionSet.from_qa_pair(
        question="How do human activities affect climate change?",
    ...
    measures = [
        SemanticCoherence(),
        EntailmentCoherence(nli_model=SimpleNLIModel()),
        HybridCoherence(semantic_weight=0.5, entailment_weight=0.5, nli_model=SimpleNLIModel()),
...
def main():
    ...
    try:
        ...
        print("📊 Coherify now provides comprehensive coherence analysis with visualization!")
