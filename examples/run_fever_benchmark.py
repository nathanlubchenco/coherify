...
def create_comprehensive_fever_mock_data() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        # Ambiguous case that could go either way
        {
            ...
            "evidence": [
                [
                    [108, 8001, "Ice_cream", 12],
                    [108, 8002, "Flavor_preference", 3]
                ]
            ]
        }
    ]
...
def run_fever_benchmark(data: List[Dict[str, Any]], 
                       use_api: bool = False,
    ...
    for i, sample in enumerate(data):
        print(f"    Processing sample {i+1}/{len(data)}: {sample.get('claim', '')[:60]}...")
                ...
                try:
                    ...
                    if "response_evaluation" in multi_result:
                        ...
                        if eval_data.get("is_correct"):
                            label_correct += 1
                        evidence_consistency_scores.append(eval_data.get("evidence_consistency", 0.0))
            ...
            for measure in measures:
                try:
                    if isinstance(measure, EvidenceBasedCoherence):
                        ...
                        sample_coherence[measure.__class__.__name__] = evidence_result["overall_coherence"]
                    else:
    ...
    if results["multi_response_results"]:
        results["fever_specific_metrics"] = {
            ...
            "evidence_consistency": float(np.mean(evidence_consistency_scores)) if evidence_consistency_scores else 0.0,
...
def analyze_fever_results(results: Dict[str, Any]):
    ...
    if fever_metrics:
        ...
        print(f"  Evidence Consistency: {fever_metrics.get('evidence_consistency', 0.0):.3f}")
        print(f"  FEVER Score: {fever_metrics.get('fever_score', 0.0):.3f}")
        print(f"  Multi-response samples: {fever_metrics.get('num_multi_response_samples', 0)}")
    ...
    if multi_results:
        ...
        print(f"  Label consistency: {consistent_count}/{len(multi_results)} ({consistent_count/len(multi_results):.1%})")
        print(f"  Good evidence usage: {evidence_usage_count}/{len(multi_results)} ({evidence_usage_count/len(multi_results):.1%})")
...
def main():
    ...
    parser.add_argument("--sample-size", type=int, default=8, help="Number of samples to evaluate")
    ...
    try:
        ...
        # Run benchmark
        results = run_fever_benchmark(data, use_api=args.use_api, sample_size=args.sample_size)
