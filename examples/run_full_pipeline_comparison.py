...
# Coherify imports
from coherify import SemanticCoherence, HybridCoherence
    ...
    return [
        {
            ...
            "correct_answers": ["Nothing happens", "You digest them", "They pass through"],
...
def run_full_pipeline_comparison(
    ...
    stage1_predictions = model_runner.generate_for_benchmark(samples, question_key="question")
    stage1_time = time.time() - start_time
    
    print(f"✅ Generated {len(stage1_predictions)} single responses in {stage1_time:.1f}s")
    ...
    k_responses_list = k_generator.generate_k_pass_dataset(samples, question_key="question")
    ...
    for responses in k_responses_list:
        if responses:
            ...
            if verbose:
                print(f"  Selected by majority (confidence: {selection.confidence:.2f})")
        else:
            stage2_predictions.append("")
            
    stage2_time = time.time() - start_time
    print(f"✅ Generated and selected {len(stage2_predictions)} responses in {stage2_time:.1f}s")
    ...
    for i, responses in enumerate(k_responses_list):
        if responses:
            ...
            if verbose:
                print(f"  Selected by coherence (score: {selection.metadata.get('best_score', 0):.3f})")
        else:
            stage3_predictions.append("")
            
    stage3_time = time.time() - start_time
    print(f"✅ Selected {len(stage3_predictions)} responses by coherence in {stage3_time:.1f}s")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run full 3-stage pipeline comparison"
    )
    parser.add_argument(
        "--model", type=str, default="gpt4-mini",
    ...
    parser.add_argument(
        "--k-responses", type=int, default=5,
