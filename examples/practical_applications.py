...
from coherify import (
    ...
    SemanticCoherence, EntailmentCoherence, HybridCoherence, ShogunjiCoherence,
    CoherenceVisualizer, CoherenceAnalyzer,
    CoherenceReranker, CoherenceRAG,
    RandomSampler, ClusterBasedApproximator, IncrementalCoherenceTracker,
    CoherenceGuidedGenerator, CoherenceFilter,
    TruthfulQAAdapter, QABenchmarkAdapter
)
...
def ai_content_quality_pipeline():
    ...
    for i, sample in enumerate(content_samples, 1):
        ...
        quality_level = "High" if avg_score > 0.6 else "Medium" if avg_score > 0.3 else "Low"
        
        print(f"  Average Score: {avg_score:.3f}")
        print(f"  Quality Assessment: {quality_level}")
        print(f"  Matches Expected: {'✅' if quality_level.lower() == sample['expected_quality'] else '❌'}")
    ...
    # Pipeline summary
    print(f"\n📊 Pipeline Summary:")
    accuracy = sum(1 for r in results if r['quality_level'].lower() == r['expected']) / len(results)
    print(f"  Quality Assessment Accuracy: {accuracy:.1%}")
    print(f"  Average Processing Score: {np.mean([r['avg_score'] for r in results]):.3f}")
    
    # Recommendations
    print(f"\n💡 Automated Recommendations:")
    for result in results:
        if result['avg_score'] < 0.3:
            print(f"  - {result['title']}: Requires major revision (score: {result['avg_score']:.3f})")
        elif result['avg_score'] < 0.6:
            print(f"  - {result['title']}: Minor improvements needed (score: {result['avg_score']:.3f})")
        else:
            print(f"  - {result['title']}: Good quality, ready for publication (score: {result['avg_score']:.3f})")
...
def scientific_paper_assessment():
    ...
    for section_name, content in paper_sections.items():
        prop_set = PropositionSet.from_qa_pair(f"Scientific paper {section_name}", content)
    ...
    for section1, section2 in section_pairs:
        ...
        print(f"  {section1.title()} → {section2.title()}: {coherence_result.score:.3f}")
    
    # Overall paper assessment
    overall_coherence = np.mean(list(section_results[s]['coherence'] for s in section_results))
    ...
    print(f"  Paper Flow Quality: {'Good' if cross_coherence > 0.5 else 'Needs Improvement'}")
    
    # Improvement recommendations
    print(f"\n💡 Improvement Recommendations:")
    weakest_section = min(section_results.items(), key=lambda x: x[1]['coherence'])
    print(f"  - Focus on improving {weakest_section[0]} section (score: {weakest_section[1]['coherence']:.3f})")
...
def educational_content_evaluation():
    ...
    for i, lesson in enumerate(lessons, 1):
        ...
        if 'pairwise_analysis' in analysis and analysis['pairwise_analysis']:
            ...
            low_coherence_pairs = [p for p in analysis['pairwise_analysis'] if p['coherence'] < 0.3]
            if low_coherence_pairs:
                print(f"  ⚠️  {len(low_coherence_pairs)} concept pairs need better connection")
    ...
    for i in range(len(lessons) - 1):
        ...
        combined_set = PropositionSet.from_qa_pair("lesson progression", combined_content)
        
        progression_coherence = HybridCoherence().compute(combined_set).score
        
        print(f"  {current_lesson['topic']} → {next_lesson['topic']}: {progression_coherence:.3f}")
    ...
    print(f"  Curriculum Quality: {'Strong' if avg_coherence > 0.6 else 'Moderate' if avg_coherence > 0.4 else 'Needs Work'}")
...
def ai_safety_hallucination_detection():
    ...
    for i, output in enumerate(ai_outputs, 1):
        ...
        if risk_level in ["Medium", "High"]:
            analyzer = CoherenceAnalyzer()
            detailed_analysis = analyzer.analyze_proposition_set(prop_set, HybridCoherence(), detailed=True)
            
            if 'least_coherent_pair' in detailed_analysis and detailed_analysis['least_coherent_pair']:
                ...
                print(f"     Content: '{least_coherent['prop1_text']}' + '{least_coherent['prop2_text']}'")
    ...
    for result in safety_results:
        ...
        if (expected_safe and detected_safe) or (not expected_safe and not detected_safe):
            correct_detections += 1
    
    accuracy = correct_detections / len(safety_results)
    print(f"  Detection Accuracy: {accuracy:.1%}")
    print(f"  Average Safety Score: {np.mean([r['safety_score'] for r in safety_results]):.3f}")
...
def production_rag_system():
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        for j, passage in enumerate(reranked_result.passages):
            ...
            print(f"  {j+1}. Coherence: {coherence_score:.3f}, Retrieval: {original_score:.3f}")
        ...
        print(f"   Improvement: {improvement['improvement']:.3f} ({improvement['relative_improvement']:.1%})")
...
def integrated_workflow_demo():
    ...
    analysis = quality_analyzer.analyze_proposition_set(prop_set, HybridCoherence(), detailed=True)
    quality_score = analysis['overall_coherence']
    
    print(f"   Quality Score: {quality_score:.3f}")
    print(f"   Assessment: {'High Quality' if quality_score > 0.6 else 'Needs Improvement'}")
    ...
    large_prop_set = PropositionSet([Proposition(text=content) for content in large_content])
    ...
    print(f"   Approximation: {approx_result.approximate_score:.3f} ({approx_result.num_clusters} clusters)")
    print(f"   Speedup: {approx_result.metadata['reduction_ratio']:.1%} computation reduction")
    ...
    def knowledge_retrieval(query: str):
        ...
        for item in knowledge_items:
            ...
            score = len(set(query.lower().split()) & set(item.lower().split())) / len(query.split())
            candidates.append(PassageCandidate(text=item, score=score))
        return sorted(candidates, key=lambda x: x.score, reverse=True)
    
    reranker = CoherenceReranker(HybridCoherence(), coherence_weight=0.6, retrieval_weight=0.4)
