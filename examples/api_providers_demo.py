...
from coherify import (
    ...
    setup_providers, get_provider, list_available_providers,
...
def api_enhanced_coherence_demo():
    ...
    try:
        ...
        measures.append(("OpenAI Semantic", APIEnhancedSemanticCoherence(
            config=config, provider=openai_provider
        )))
        measures.append(("OpenAI Hybrid", APIEnhancedHybridCoherence(
            config=config, provider=openai_provider
        )))
    except:
    ...
    try:
        ...
        measures.append(("Anthropic Semantic", APIEnhancedSemanticCoherence(
            config=config, provider=anthropic_provider
        )))
        measures.append(("Anthropic Hybrid", APIEnhancedHybridCoherence(
            config=config, provider=anthropic_provider
        )))
    except:
        ...
        try:
            ...
            if "api_enhancement" in result.details:
                ...
                if "embeddings_coherence" in enhancement:
                    print(f"  API Embeddings Score: {enhancement['embeddings_coherence']:.3f}")
                
                if "temperature_variance" in enhancement:
                    print(f"  Temperature Variance Score: {enhancement['temperature_variance']:.3f}")
...
def temperature_variance_demo():
    ...
    try:
        ...
        test_prop_set = PropositionSet([
            Proposition("Machine learning enables computers to learn patterns from data."),
            Proposition("Neural networks are inspired by biological brain structures."),
            Proposition("Deep learning uses multiple layers for complex pattern recognition.")
        ])
        ...
        enhanced_measure = APIEnhancedSemanticCoherence(config=config, provider=provider)
...
def reasoning_model_demo():
    ...
    try:
        ...
        if reasoning_analysis:
            print(f"  Reasoning Score: {reasoning_analysis.get('reasoning_score', 'N/A')}")
            print(f"  Has Reasoning Trace: {'✅' if reasoning_analysis.get('has_trace') else '❌'}")
...
def multi_provider_comparison():
    ...
    print(f"🔍 Comparing coherence measures across {len(available_providers)} providers")
...
def production_configuration_demo():
    ...
    try:
        ...
        test_text = "AI systems require careful evaluation for coherence and reliability."
