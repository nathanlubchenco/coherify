...
from coherify.generation import (
    ...
    CoherenceFilter, MultiStageFilter, AdaptiveCoherenceFilter,
...
def create_mock_language_model():
    ...
    vocabularies = {
        "machine_learning": {
            "high_prob": ["learning", "algorithm", "data", "model", "training", "neural", "network"],
            "medium_prob": ["analysis", "prediction", "accuracy", "optimization", "feature", "pattern"],
            "low_prob": ["science", "technology", "system", "process", "method", "research"]
        },
        "climate_science": {
            "high_prob": ["climate", "temperature", "carbon", "emissions", "warming", "atmosphere"],
            "medium_prob": ["greenhouse", "effects", "ocean", "weather", "environmental", "pollution"],
            "low_prob": ["science", "research", "study", "data", "analysis", "global"]
        },
        "general": {
            "high_prob": ["the", "and", "of", "to", "a", "in", "that", "is", "for", "with"],
            "medium_prob": ["this", "can", "will", "are", "be", "have", "from", "they", "which"],
            "low_prob": [".", ",", "!", "?", "however", "therefore", "moreover", "furthermore"]
        }
    }
    
    def generate_candidates(context: str, current_text: str, num_candidates: int) -> List[Tuple[str, float]]:
...
def beam_search_demo():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        top_candidates = sorted(result.all_candidates, key=lambda x: x.combined_score, reverse=True)[:3]
        print(f"  Top alternatives:")
        for j, candidate in enumerate(top_candidates[:3]):
            if candidate != result.best_candidate:
                print(f"    {j+1}. '{candidate.text}' (coherence: {candidate.coherence_score:.3f})")
...
def filtering_demo():
    ...
    for i, (candidate, score) in enumerate(zip(basic_result.passed_candidates, basic_result.coherence_scores)):
    ...
    multi_filter = MultiStageFilter(
        ...
        stages=[
            ...
            {"name": "excellence", "threshold": 0.6, "description": "Excellence filter"}
        ]
    )
    ...
    for stage_name, stage_result in multi_results.items():
        print(f"  {stage_name.capitalize()}: {len(stage_result.passed_candidates)} candidates")
    ...
    for round_num in range(3):
        ...
        print(f"  Round {round_num + 1}: Threshold {adaptive_filter.min_coherence_threshold:.2f}, "
...
def streaming_guidance_demo():
    ...
    streaming_tokens = [
        "Deep", "learning", "models", "use", "neural",
        "networks", "to", "process", "complex", "data",
        "patterns", "and", "make", "accurate", "predictions",
        "about", "future", "outcomes", "in", "various",
...
def main():
    ...
    try:
        ...
        print("🎯 Coherify now provides comprehensive generation guidance with coherence!")
