...
def create_mock_retrieval_function():
    ...
    passages_db = [
        ("Machine learning is a subset of artificial intelligence that enables computers to learn without being explicitly programmed.", 0.95),
        ("Neural networks are computing systems inspired by biological neural networks and are used in machine learning.", 0.90),
        ("Deep learning uses neural networks with multiple layers to model complex patterns in data.", 0.88),
        ("Supervised learning algorithms learn from labeled training data to make predictions on new data.", 0.85),
        ("Unsupervised learning finds hidden patterns in data without using labeled examples.", 0.82),
        ("Reinforcement learning trains agents to make decisions by rewarding good actions and penalizing bad ones.", 0.80),
        ("The weather today is sunny with a high of 75 degrees Fahrenheit.", 0.15),  # Irrelevant
        ("Overfitting occurs when a model learns the training data too well and performs poorly on new data.", 0.78),
        ("Feature engineering involves selecting and transforming variables for machine learning models.", 0.75),
        ("Cross-validation is a technique for assessing how well a model generalizes to unseen data.", 0.73),
        ("Python is a programming language commonly used for machine learning and data science.", 0.70),
        ("Gradient descent is an optimization algorithm used to minimize loss functions in machine learning.", 0.72),
        ("Data preprocessing includes cleaning, transforming, and preparing data for machine learning algorithms.", 0.68),
        ("Basketball is a sport played by two teams of five players each on a rectangular court.", 0.10),  # Irrelevant
        ("Bias in machine learning refers to systematic errors that can affect model predictions.", 0.65),
    ]
...
def basic_reranking_demo():
    ...
    reranked_result = reranker.rerank(
        query=query,
        passages=initial_candidates,
        top_k=5
    )
    
    print(f"\n🔄 Coherence-Reranked Results (Top 5):")
    for i, (passage, coh_score, orig_score) in enumerate(zip(
...
def coherence_guided_retrieval_demo():
    ...
    expanded_query = retriever.expand_query(
        query=query,
        domain_knowledge=["artificial intelligence", "deep learning", "backpropagation", "neurons"]
    )
    ...
    for i, candidate in enumerate(coherence_results):
        ...
        print(f"  {i+1}. Retrieval: {candidate.score:.3f}, Coherence: {coherence_score:.3f}")
        print(f"     Text: {candidate.text[:60]}...")
    
    # Evaluate retrieval quality
    evaluation = retriever.evaluate_retrieval_coherence(
        query=query,
        candidates=coherence_results,
        top_k=5
    )
...
def iterative_refinement_demo():
    ...
    retriever = CoherenceGuidedRetriever(
        coherence_measure=HybridCoherence(),
        coherence_filter_threshold=0.2
    )
...
def comparative_analysis_demo():
    ...
    strategies = {
        "High Coherence Weight": CoherenceReranker(coherence_weight=0.8, retrieval_weight=0.2),
        "Balanced": CoherenceReranker(coherence_weight=0.5, retrieval_weight=0.5),
        "High Retrieval Weight": CoherenceReranker(coherence_weight=0.2, retrieval_weight=0.8),
        "No Reranking": None
    }
