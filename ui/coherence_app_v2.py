...
try:
    from .performance import (
        ModelManager, get_cached_example_texts, ProgressTracker,
...
def render_workbench():
    ...
    st.markdown('<div class="main-header">Coherence Analysis Platform</div>', 
                unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        performance_mode = st.radio(
            "Analysis Mode",
            ["Fast (Semantic only)", "Balanced (Fast + NLI)", "Complete (All measures)", "Advanced (+ Shogenji)", "API Enhanced"],
            index=0,
        ...
        if performance_mode == "API Enhanced":
            ...
            if api_provider and api_provider != "None":
                ...
                if api_key_status:
                    ...
                    st.info(f"💡 Set your API key: `export {key_env_var}=your_key_here`")
        
        # Measure selection based on performance mode
        fast_measures = get_fast_measures()
        slow_measures = get_slow_measures()
        advanced_measures = get_advanced_measures()
        api_measures = get_api_enhanced_measures(api_provider, api_model) if api_key_status else {}
        ...
        if performance_mode == "Fast (Semantic only)":
            ...
            available_measures = {**fast_measures, **slow_measures, **advanced_measures, **api_measures}
        else:
            available_measures = {**fast_measures, **slow_measures}
        ...
        if not available_measures:
            ...
            st.info("💡 Try refreshing the page or check your internet connection for model downloads.")
        else:
            ...
            expected_api = [f"{api_provider.title()} Enhanced"] if api_provider and api_provider != "None" else []
            ...
            if performance_mode == "Fast (Semantic only)":
                ...
                expected = expected_fast + expected_slow + expected_advanced + expected_api
            else:
                expected = expected_fast + expected_slow
    ...
    with col1:
        ...
        if selected_example == "Custom":
            num_props = st.number_input("Number of statements", min_value=2, max_value=8, value=3)
            ...
            for i, prop in enumerate(example_data["propositions"]):
                edited_prop = st.text_input(f"Statement {i+1}", value=prop, key=f"prop_{i}")
    ...
    with col2:
        ...
        if st.session_state.coherence_results:
            ...
            unbounded_measures = [name for name, data in results.items() 
...
def render_benchmark_dashboard():
    ...
    st.markdown('<div class="main-header">Benchmark Performance</div>', 
                unsafe_allow_html=True)
    ...
    benchmark_data = {
        "FEVER": {"baseline": 0.654, "coherence": 0.721, "samples": 16821, "metric": "F1"},
        "TruthfulQA": {"baseline": 0.438, "coherence": 0.502, "samples": 817, "metric": "Accuracy"},
        "FaithBench": {"baseline": 0.583, "coherence": 0.641, "samples": 1200, "metric": "AUC"},
        "SelfCheckGPT": {"baseline": 0.692, "coherence": 0.734, "samples": 2000, "metric": "Precision@K"}
    }
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Navigate",
        ["Analysis Workbench", "Benchmark Results"]
    )
