...
try:
    from .config import (
        UI_CONFIG, MEASURE_COLORS, DEFAULT_EXAMPLES, BENCHMARK_DATA,
        MEASURE_EXPLANATIONS, UI_CONTENT, CHART_CONFIG
    )
except ImportError:
...
def get_example_texts():
    ...
    examples = {"Custom": {"context": "", "propositions": [], "description": "Enter your own text"}}
...
def create_pairwise_heatmap(results, propositions):
    ...
    labels = [f"P{i+1}: {prop[:30]}..." if len(prop) > 30 else f"P{i+1}: {prop}" 
...
def render_workbench():
    ...
    st.markdown('<div class="main-header">🧠 Coherence Analysis Workbench</div>', 
                unsafe_allow_html=True)
    ...
    with col1:
        ...
        if selected_example == "Custom":
            ...
            num_props = st.number_input("Number of propositions:", min_value=1, max_value=10, value=3)
            ...
            for i, prop in enumerate(example_data["propositions"]):
                edited_prop = st.text_input(f"Proposition {i+1}:", value=prop, key=f"prop_{i}")
        ...
        with col2:
            ...
            if len(propositions) >= 2:
                ...
                if heatmap_fig:
                    ...
                    st.info("Pairwise similarity data not available for selected measures.")

def render_benchmark_dashboard():
    """Render the benchmark performance dashboard."""
    st.markdown('<div class="main-header">📊 Benchmark Performance Dashboard</div>', 
                unsafe_allow_html=True)
    ...
    avg_coherence = np.mean([data["coherence_guided"] for data in benchmark_data.values()])
    ...
    # Detailed comparison
    st.markdown('<div class="section-header">🏆 Detailed Performance Comparison</div>', 
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">📈 Improvement Analysis</div>', 
                unsafe_allow_html=True)
    ...
    # Benchmark details
    st.markdown('<div class="section-header">📋 Benchmark Details</div>', 
                unsafe_allow_html=True)
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Navigate to:",
        ["🔬 Coherence Workbench", "📊 Benchmark Dashboard"]
    )
