    ...
    def _get_reports_list(self) -> List[Dict[str, Any]]:
        ...
        for json_file in self.results_dir.glob("*.json"):
            try:
                ...
                if (
                    "truthful_score" in native_metrics
                    and native_metrics["truthful_score"] is not None
                ):
                    primary_metric_value = native_metrics["truthful_score"]
                    primary_metric_label = "Truthfulness"
                elif (
                    "baseline_accuracy" in native_metrics
                    and native_metrics["baseline_accuracy"] is not None
                ):
...
def start_result_server(
    results_dir: str = "results", port: int = 8080, open_browser: bool = True
) -> ResultViewer:
    """
