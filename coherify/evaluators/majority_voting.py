...
class MajorityVotingEvaluator:
    ...
    def evaluate_single_with_voting(
        ...
        final_answer, vote_distribution, confidence = self._apply_majority_voting(
            individual_runs
        )
        ...
        for i, sample in enumerate(dataset):
            ...
            if progress_callback:
                ...
                def sample_progress_callback(run_idx, total_runs):
                    ...
                    progress_callback(
                        int(overall_progress * total_samples), total_samples
                    )

            voting_result = self.evaluate_single_with_voting(
                sample, progress_callback=sample_progress_callback
            )
        ...
        stats.update(
            {
                ...
                "avg_time_per_sample": (
                    total_time / total_samples if total_samples > 0 else 0
                ),
                "avg_time_per_run": (
                    total_time / (total_samples * self.k_runs)
                    if total_samples > 0
                    else 0
                ),
            }
    ...
    def _weighted_majority_voting(
        ...
        confidence = (
            answer_weights[final_answer] / total_weight if total_weight > 0 else 0.0
        )
    ...
    def _compute_dataset_statistics(
        ...
        stats = {
            ...
            "mean_coherence": (
                np.mean(all_coherence_scores) if all_coherence_scores else 0.0
            ),
            "std_coherence": (
                np.std(all_coherence_scores) if all_coherence_scores else 0.0
            ),
            "mean_eval_time": np.mean(eval_times) if eval_times else 0.0,
            "total_individual_runs": sum(
                len(r.individual_runs) for r in voting_results
            ),
            "successful_runs": sum(
                len([run for run in r.individual_runs if run.get("valid", False)])
...
def create_majority_voting_evaluator(
    base_evaluator: Any, k_runs: int = 5, strategy: str = "simple"
    ...
    return MajorityVotingEvaluator(
        base_evaluator=base_evaluator, k_runs=k_runs, voting_strategy=strategy
    )
