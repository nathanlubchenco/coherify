...
class KRunBenchmarkEvaluator:
    ...
    def evaluate_dataset(
        ...
        if progress_callback:
            progress_callback(0.0, f"Starting K-run evaluation (K={self.config.k_runs})")
        ...
        if progress_callback:
            progress_callback(1.0, f"K-run evaluation completed ({successful}/{len(voting_results)} successful)")
    ...
    def _evaluate_parallel(
        ...
        with ThreadPoolExecutor(max_workers=self.config.max_workers) as executor:
            # Submit all samples
            future_to_sample = {
                executor.submit(self._evaluate_single_with_retry, i, sample): (i, sample)
            ...
            for future in as_completed(future_to_sample):
                ...
                if progress_callback:
                    ...
                    progress_callback(progress, f"Completed {completed_samples}/{total_samples} samples")
    ...
    def _evaluate_single_with_retry(self, sample_idx: int, sample: Dict[str, Any]) -> VotingResult:
        """Evaluate single sample with retry logic."""
        max_attempts = self.config.max_retries + 1 if self.config.retry_failed_runs else 1
        last_exception = None
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self.config.cache_intermediate and cache_key in self._intermediate_cache:
                    return self._intermediate_cache[cache_key]
                
                # Evaluate with majority voting
                voting_result = self.majority_evaluator.evaluate_single_with_voting(sample)
                
                # Cache result if successful
                if self.config.cache_intermediate and voting_result.final_answer is not None:
    ...
    def create_evaluation_report(self, k_run_result: KRunResult) -> str:
        ...
        report_lines = [
            ...
            f"- **Parallelization:** {config.max_workers} workers" if config.parallel_execution else "- **Execution:** Sequential"
        ]
