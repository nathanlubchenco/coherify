...
class PropositionSampler(ABC):
    ...
    @abstractmethod
    def get_sampling_weights(self, propositions: List[Proposition]) -> Optional[List[float]]:
...
class StratifiedSampler(PropositionSampler):
    ...
    def sample(self, 
        ...
        for stratum_props in strata:
            ...
            stratum_sample_size = max(1, int((stratum_size / total_props) * sample_size))
    ...
    def _create_strata(self, propositions: List[Proposition]) -> List[List[Proposition]]:
        ...
        if self.strata_key == "length":
            ...
            for prop in propositions:
                bin_idx = min(int((len(prop.text) - min_len) / bin_size), self.num_strata - 1)
            ...
            for prop, complexity in zip(propositions, complexities):
                bin_idx = min(int((complexity - min_comp) / bin_size), self.num_strata - 1)
                strata[bin_idx].append(prop)
            ...
            for candidate in remaining:
                ...
                for sampled_prop in sampled:
                    ...
                    if hasattr(self.coherence_measure, 'encoder'):
                        ...
                        try:
                            embeddings = self.coherence_measure.encoder.encode([sampled_prop.text, candidate.text])
...
class SamplingBasedApproximator:
    ...
    def approximate_coherence(self,
                            prop_set: PropositionSet,
                            sample_size: int,
                            compute_confidence: bool = True) -> SamplingResult:
        """
        ...
        return SamplingResult(
            ...
            metadata={
                ...
                "bootstrap_samples": self.num_bootstrap_samples if compute_confidence else 0
            }
        )
    ...
    def evaluate_sampling_quality(self,
        ...
        for sample_size in sample_sizes:
            ...
            if true_score is not None:
                results[sample_size]["absolute_error"] = abs(np.mean(trial_scores) - true_score)
                results[sample_size]["relative_error"] = abs(np.mean(trial_scores) - true_score) / max(true_score, 1e-8)
