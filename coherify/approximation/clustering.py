...
class ClusterBasedApproximator(PropositionClusterer):
    ...
    def approximate_coherence(self,
                            ...
                            target_clusters: Optional[int] = None) -> ClusteringResult:
        """
        ...
        n_clusters = target_clusters or self.num_clusters or self._determine_optimal_clusters(n_props)
    ...
    def _get_cluster_representatives(self,
                                   ...
                                   n_clusters: int) -> List[Proposition]:
        ...
        for cluster_id in range(n_clusters):
            ...
            if self.cluster_selection_strategy == "centroid":
                ...
                for i, emb1 in enumerate(cluster_embeddings):
                    total_distance = sum(np.linalg.norm(emb1 - emb2) for emb2 in cluster_embeddings)
    ...
    def _compute_cluster_coherences(self,
                                  propositions: List[Proposition],
                                  cluster_labels: np.ndarray,
                                  n_clusters: int) -> Tuple[List[float], List[int]]:
        ...
        for cluster_id in range(n_clusters):
            ...
            if len(cluster_props) <= 1:
                cluster_coherences.append(1.0)  # Single proposition is perfectly coherent
            else:
...
class HierarchicalCoherenceApproximator(PropositionClusterer):
    ...
    def approximate_coherence(self,
                            prop_set: PropositionSet,
                            target_depth: Optional[int] = None) -> ClusteringResult:
        """
    ...
    def _build_hierarchy(self, 
                        propositions: List[Proposition], 
                        max_depth: int) -> List[List[Dict[str, Any]]]:
