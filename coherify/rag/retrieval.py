...
from coherify.core.base import CoherenceMeasure, PropositionSet, CoherenceResult
...
class CoherenceGuidedRetriever:
    ...
    def expand_query(self, 
                     ...
                     domain_knowledge: Optional[List[str]] = None) -> CoherenceGuidedQuery:
        """
        ...
        if self.query_expansion_strategy == "semantic":
            ...
            raise ValueError(f"Unknown expansion strategy: {self.query_expansion_strategy}")
    
    def _semantic_expansion(self, 
                           query: str,
                           context: Optional[str] = None,
                           domain_knowledge: Optional[List[str]] = None) -> CoherenceGuidedQuery:
        ...
        semantic_expansions = {
            "machine learning": ["AI", "artificial intelligence", "algorithms", "neural networks"],
        ...
        if domain_knowledge:
            ...
            expansion_terms.extend([term for term, _ in domain_scores[:self.max_expansion_terms]])
    ...
    def _keyword_expansion(self, 
        ...
        unique_expansions = list(set(expansion_terms))[:self.max_expansion_terms]
        expanded_query = f"{query} {' '.join(unique_expansions)}" if unique_expansions else query
    ...
    def _hybrid_expansion(self, 
        ...
        all_terms = semantic_expanded.coherence_keywords + keyword_expanded.coherence_keywords
    ...
    def retrieve_with_coherence_filtering(self,
                                        query: str,
                                        retrieval_function: Callable[[str], List[PassageCandidate]],
                                        max_candidates: int = 20,
                                        context: Optional[str] = None) -> List[PassageCandidate]:
        """
    ...
    def iterative_coherence_retrieval(self,
                                    query: str,
                                    retrieval_function: Callable[[str], List[PassageCandidate]],
                                    max_iterations: int = 3,
                                    convergence_threshold: float = 0.05) -> Tuple[List[PassageCandidate], Dict[str, Any]]:
        """
    ...
    def _refine_query_from_results(self, 
                                  original_query: str,
                                  top_candidates: List[PassageCandidate]) -> str:
        ...
        meaningful_words = [w for w in words if len(w) > 4 and w not in {
            ...
            "could", "should", "there", "where", "when", "what", "which"
        }]
    ...
    def evaluate_retrieval_coherence(self,
                                   query: str,
        ...
        if hasattr(self.coherence_measure, 'encoder'):
            try:
                ...
                diversity = 1.0 - np.mean(upper_triangle) if len(upper_triangle) > 0 else 0.0
            except:
                diversity = None
        else:
            diversity = None
