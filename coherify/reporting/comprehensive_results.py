    ...
    def _generate_markdown(self) -> str:
        ...
        if self.native_metrics:
            ...
            if self.benchmark_primary_metric:
                ...
                lines.append(
                    f"- **Primary Metric ({metric_name})**: {metric_value:.3f}"
                )
            ...
            if self.performance_validation:
                for metric_name, validation in self.performance_validation.items():
                    if not validation.get("is_realistic", True):
                        lines.append(
                            f"⚠️  **Performance Warning ({metric_name})**: {validation.get('explanation', '')}"
                        )
                    elif "expectations" in validation:
                        exp = validation["expectations"]
                        if "best_model" in exp:
                            lines.append(
                                f"ℹ️  **Research Context**: Best published {metric_name} {exp['best_model']:.1%}"
                            )

            if "truthful_score" in self.native_metrics:
                lines.append(
                    f"- **Truthfulness**: {self.native_metrics['truthful_score']:.3f}"
                )
                lines.append(
                    f"- **Informativeness**: {self.native_metrics.get('informative_score', 0):.3f}"
                )

            if "baseline_accuracy" in self.native_metrics:
                lines.append(
                    f"- **Baseline Accuracy**: {self.native_metrics['baseline_accuracy']:.3f}"
                )

            if (
                "coherence_filtered_accuracy" in self.native_metrics
                and self.native_metrics["coherence_filtered_accuracy"] is not None
            ):
                lines.append(
                    f"- **Coherence-Filtered Accuracy**: {self.native_metrics['coherence_filtered_accuracy']:.3f}"
                )
                if self.native_metrics.get("improvement") is not None:
                    ...
                    if (
                        ...
                        and "coherence_improvement" in self.performance_validation
                    ):
                        exp_range = self.performance_validation[
                            "coherence_improvement"
                        ].get("expected_range")
                        if (
                            exp_range
                            and isinstance(exp_range, (list, tuple))
                            and len(exp_range) == 2
                        ):
                            lines.append(
                                f"- **Expected Coherence Improvement**: {exp_range[0]:.1%}-{exp_range[1]:.1%}"
                            )
            lines.append("")
        ...
        if self.min_coherence is not None and self.max_coherence is not None:
            lines.append(
                f"- **Range**: {self.min_coherence:.3f} - {self.max_coherence:.3f}"
            )
        lines.append("")
        ...
        lines.append(
            f"- **Throughput**: {self.throughput_samples_per_second:.2f} samples/sec"
        )
        ...
        if self.benchmark_context.description:
            ...
            if self.benchmark_context.human_performance:
                lines.append(
                    f"**Human Performance**: {self.benchmark_context.human_performance:.3f}"
                )
            if self.benchmark_context.state_of_art_performance:
                lines.append(
                    f"**State-of-the-Art**: {self.benchmark_context.state_of_art_performance:.3f}"
                )
            lines.append("")
...
class BenchmarkReporter:
    ...
    def create_report(
        ...
        report = BenchmarkReport(
            ...
            performance_validation=self._validate_performance(
                benchmark_name, raw_results.get("native_metrics")
            ),
    ...
    def save_report(self, report: BenchmarkReport) -> Tuple[Path, Path]:
        ...
        base_filename = (
            f"{report.benchmark_name}_{timestamp}_{report.evaluation_id[:8]}"
        )
    ...
    def _calculate_coherence_statistics(
        ...
        if "detailed_results" in raw_results:
            all_scores = [
                r.get("coherence_score", 0) for r in raw_results["detailed_results"]
            ]
    ...
    def _get_benchmark_context(self, benchmark_name: str) -> BenchmarkContext:
        ...
        return self._benchmark_contexts.get(
            benchmark_name.lower(), BenchmarkContext(benchmark_name=benchmark_name)
        )

    def _validate_performance(
        self, benchmark_name: str, native_metrics: Optional[Dict[str, Any]]
