...
class SelfCheckGPTAdapter(BenchmarkAdapter):
    ...
    def _adapt_multi_sample(self, sample: Dict[str, Any], prompt: str) -> PropositionSet:
        ...
        if len(responses) < self.min_samples:
            raise ValueError(f"Need at least {self.min_samples} samples, got {len(responses)}")
    ...
    def _adapt_sentence_level(self, sample: Dict[str, Any], prompt: str) -> PropositionSet:
    ...
    def create_consistency_groups(self, sample: Dict[str, Any]) -> Dict[str, PropositionSet]:
        """
...
class SelfCheckGPTEvaluator:
    ...
    def evaluate_consistency(self, sample: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluate consistency of multiple generations."""
        ...
        if sample_coherences:
            ...
            evaluation["mean_sample_coherence"] = np.mean(list(sample_coherences.values()))
            evaluation["sample_coherence_std"] = np.std(list(sample_coherences.values()))
        
        # Consistency metrics
        if len(sample_coherences) > 1:
            coherence_values = list(sample_coherences.values())
            evaluation["coherence_consistency"] = 1.0 - np.std(coherence_values)  # Higher when more consistent
    ...
    def _score_sentence_reliability(self, sentence: str) -> float:
        ...
        specificity_score = sum(1 for word in words if word.lower() in specific_terms) / len(words)
        
        # Combine scores
        return (length_score + specificity_score) / 2.0
    
    def compare_generation_strategies(self, samples: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
