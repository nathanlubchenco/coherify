...
from .adapters import BenchmarkAdapter, QABenchmarkAdapter
...
class APIEnhancedQAAdapter(QABenchmarkAdapter):
    ...
    def _enhance_with_api_generation(
        ...
        try:
            ...
            print(f"Warning: API enhancement failed for question '{question[:50]}...': {e}")
    ...
    def _expand_answer(self, question: str, original_answer: str) -> Optional[ModelResponse]:
...
class APIBenchmarkEvaluator:
    ...
    def evaluate_sample(self, sample: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for measure in self.coherence_measures:
            try:
                ...
                print(f"Warning: Coherence evaluation failed for {measure.__class__.__name__}: {e}")
                results["coherence_scores"][measure.__class__.__name__] = {
                    "score": 0.0,
                    "error": str(e)
                }
    ...
    def evaluate_dataset(
        ...
        results["aggregate_scores"] = self._compute_aggregate_scores(results["sample_results"])
        results["api_statistics"] = self._compute_api_statistics(results["sample_results"])
        
        return results
    
    def _compute_aggregate_scores(self, sample_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _compute_api_statistics(self, sample_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if api_enhanced_samples > 0:
            stats["average_generations_per_sample"] = total_generations / api_enhanced_samples
    ...
    def create_evaluation_report(self, evaluation_results: Dict[str, Any]) -> str:
        ...
        report.append(f"Total Evaluation Time: {evaluation_results['total_evaluation_time']:.2f}s")
        
        # API Statistics
        api_stats = evaluation_results["api_statistics"]
        report.append(f"\n--- API Enhancement Statistics ---")
        report.append(f"Samples with API Enhancement: {api_stats['samples_with_api_enhancement']}")
        report.append(f"Total API Generations: {api_stats['total_api_generations']}")
        report.append(f"Avg Generations per Sample: {api_stats['average_generations_per_sample']:.1f}")
