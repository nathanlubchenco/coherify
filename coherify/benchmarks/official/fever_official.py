...
class FEVEROfficialEvaluator:
    ...
    def __init__(self, require_evidence: bool = True, strict_scoring: bool = True):
        """
    ...
    def evaluate_dataset(
        ...
        if HAS_FEVER_SCORER and self.strict_scoring:
            return self._evaluate_with_official_scorer(
                predictions, gold_labels, verbose
            )
        else:
            return self._evaluate_with_approximate_scoring(
                predictions, gold_labels, verbose
            )
    ...
    def _evaluate_with_official_scorer(
        ...
        strict_score, label_accuracy, precision, recall, f1 = fever_score(
            formatted_predictions, formatted_gold
        )
        ...
        for i, (pred, gold) in enumerate(zip(predictions, gold_labels)):
            ...
            evidence_correct = self._check_evidence_correct(
                pred.get("predicted_evidence", []), gold.get("evidence", [])
            )
    ...
    def _evaluate_with_approximate_scoring(
        ...
        if verbose:
            print(
                "Using approximate FEVER scoring (install fever-scorer for official)..."
            )
        ...
        return FEVEROfficialResult(
            ...
            oracle_accuracy=(
                label_correct_count / n if n > 0 else 0
            ),  # Same as label acc
            per_sample_results=per_sample_results,
            method="approximate",
    ...
    def _check_evidence_correct(
        self, predicted_evidence: List[List], gold_evidence_sets: List[List[List]]
    ) -> bool:
        """
    ...
    def format_predictions_for_evaluation(
        claims: List[str], labels: List[str], evidence: Optional[List[List]] = None
    ) -> List[Dict[str, Any]]:
        """
...
def evaluate_fever_official(
    predictions: List[Dict[str, Any]], gold_labels: List[Dict[str, Any]]
) -> Dict[str, float]:
    """
