...
class SelfCheckGPTOfficialEvaluator:
        ...
        if method in ["bertscore", "combined"]:
            if not HAS_BERTSCORE:
                ...
                self.bert_scorer = BERTScorer(
                    model_type=bert_model, lang="en", rescale_with_baseline=True
                )
    ...
    def _evaluate_bertscore(
        ...
        for i, (original, additional) in enumerate(
            zip(original_samples, additional_samples)
        ):
            ...
            if not additional:
                ...
                P, R, F1 = self.bert_scorer.score(
                    [original] * len(additional), additional
                )
            ...
            per_sample_results.append(
                {
                    ...
                    "ground_truth": (
                        ground_truth_labels[i] if i < len(ground_truth_labels) else None
                    ),
                    "method": "bertscore",
                }
            )
        ...
        auc_pr, auc_roc = self._calculate_auc_scores(
            consistency_scores, ground_truth_labels
        )
    ...
    def _calculate_auc_scores(
        self, consistency_scores: List[float], ground_truth_labels: List[int]
    ) -> Tuple[float, float]:
    ...
    def validate_against_published():
        ...
        validation_info = {
            ...
            "human": {
                "auc_pr": None,
                "description": "N/A - consistency detection task",
            },
