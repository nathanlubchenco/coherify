...
class BenchmarkPerformanceExpectations:
    ...
    @classmethod
    def is_performance_realistic(
        cls, benchmark_name: str, performance: float
    ) -> Tuple[bool, str]:
        """
        ...
        if performance > best_model + 0.2:  # 20% above best model is unrealistic
            return (
                False,
                f"Performance {performance:.1%} significantly exceeds best published result {best_model:.1%}",
            )
        elif performance < 0.1:  # Below 10% is likely a bug
            return (
                False,
                f"Performance {performance:.1%} is unrealistically low, check evaluation logic",
            )
        else:
            return (
                True,
                f"Performance {performance:.1%} is within realistic range (best: {best_model:.1%})",
            )
...
class TruthfulQAMetrics:
    ...
    def calculate_metrics(
        ...
        metrics.informative_score = (
            informative_count / total_count if total_count > 0 else 0
        )
        ...
        if coherence_scores:
            ...
            if filtered_total > 0:
                ...
                metrics.improvement = (
                    metrics.coherence_filtered_accuracy - metrics.baseline_accuracy
                )

        # Add generation metrics if available
        if HAS_ROUGE:
            rouge_scores = TruthfulQAMetrics._calculate_rouge_scores(
                predictions, samples
            )
...
class SelfCheckGPTMetrics:
    ...
    def calculate_metrics(
        ...
        if coherence_scores:
            ...
            coherence_predictions = [
                score < coherence_threshold for score in coherence_scores
            ]

            if HAS_SKLEARN:
                metrics.coherence_filtered_accuracy = accuracy_score(
                    labels, coherence_predictions
                )
                metrics.improvement = (
                    metrics.coherence_filtered_accuracy - metrics.baseline_accuracy
                )
    ...
    @staticmethod
    def check_consistency_bertscore(
        main_response: str, sampled_responses: List[str]
    ) -> float:
        """
        ...
        try:
            ...
            return SelfCheckGPTMetrics._fallback_similarity_score(
        ...
        try:
            P, R, F1 = bert_score(
                main_responses, sampled_responses, lang="en", verbose=False
            )
    ...
    def check_consistency_nli(
        ...
        try:
            ...
            return SelfCheckGPTMetrics._fallback_nli_score(
                main_response, sampled_responses
            )
        ...
        for sampled in sampled_responses:
            try:
                ...
                result = nli_pipeline({"text": main_response, "text_pair": sampled})
                ...
                similarity = SelfCheckGPTMetrics._calculate_text_similarity(
                    main_response, sampled
                )
    ...
    @staticmethod
    def check_consistency_ngram(
        main_response: str, sampled_responses: List[str], n: int = 2
    ) -> float:
        """
    ...
    def check_consistency_qa_based(
        main_response: str, sampled_responses: List[str], original_question: str
    ) -> float:
        """
        ...
        for sampled in sampled_responses:
            ...
            if not main_terms or not sampled_terms:
                ...
                consistency_scores.append(
                    overlap / len(main_terms.union(sampled_terms))
                )
...
class GeneralQAMetrics:
    ...
    def calculate_metrics(
        ...
        if coherence_scores:
            filtered_matches = []
            for i, (pred, ref, score) in enumerate(
                zip(predictions, references, coherence_scores)
            ):
                if score >= coherence_threshold:
                    filtered_matches.append(pred.strip().lower() == ref.strip().lower())

            if filtered_matches:
                metrics.coherence_filtered_accuracy = sum(filtered_matches) / len(
                    filtered_matches
                )
                metrics.improvement = (
                    metrics.coherence_filtered_accuracy - metrics.baseline_accuracy
