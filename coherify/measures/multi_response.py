...
class MultiResponseCoherenceMeasure(CoherenceMeasure):
    ...
    def compute_multi_response(self, 
                             prompt: str, 
                             context: Optional[str] = None) -> MultiResponseResult:
        """
        ...
        confidence_score = self._compute_confidence_score(pairwise_coherences, coherence_scores)
    ...
    def _generate_responses(self, prompt: str, context: Optional[str] = None) -> List[str]:
    ...
    def _generate_synthetic_responses(self, prompt: str, context: Optional[str] = None) -> List[str]:
    ...
    def _get_temperatures(self) -> List[float]:
        ...
        if self.config.temperature_strategy == "uniform":
            ...
            raise ValueError(f"Unknown temperature strategy: {self.config.temperature_strategy}")
    
    def _responses_to_proposition_sets(self, responses: List[str], context: Optional[str]) -> List[PropositionSet]:
    ...
    def _compute_individual_coherences(self, prop_sets: List[PropositionSet]) -> List[float]:
    ...
    def _compute_confidence_score(self, 
                                pairwise_coherences, 
                                individual_scores: List[float]) -> float:
        """
...
class TemperatureVarianceCoherence(MultiResponseCoherenceMeasure):
    ...
    def evaluate_temperature_consistency(self, 
                                       prompt: str,
        ...
        return {
            ...
            "consistency_verdict": "consistent" if result.consistency_score > self.config.consistency_threshold else "inconsistent"
        }
...
class SelfConsistencyCoherence(MultiResponseCoherenceMeasure):
    ...
    def __init__(self, 
        ...
        if config is None:
            config = MultiResponseConfig(
                num_responses=3,
                temperature_range=(0.7, 0.7),  # Fixed temperature for pure self-consistency
    ...
    def evaluate_self_consistency(self, 
        ...
        return {
            ...
            "is_self_consistent": result.consistency_score > self.config.consistency_threshold,
