...
class APIEnhancedSemanticCoherence(SemanticCoherence):
    ...
    def _compute_api_enhancement(self, prop_set: PropositionSet) -> Dict[str, Any]:
        ...
        try:
            ...
            if embeddings:
                ...
                enhancement_data["embeddings_coherence"] = (
                    self._compute_embeddings_coherence(embeddings)
                )

            # Temperature variance analysis
            if self.config.use_temperature_variance:
                enhancement_data["temperature_variance"] = (
                    self._compute_temperature_variance(prop_set)
                )

            # Reasoning analysis for supported models
            if self.config.enable_reasoning_trace:
                enhancement_data["reasoning_analysis"] = (
                    self._compute_reasoning_analysis(prop_set)
                )
    ...
    def _compute_reasoning_analysis(
        self, prop_set: PropositionSet
...
"""
    ...
    def _combine_scores(
        self, base_score: float, enhancement_data: Dict[str, Any]
    ) -> float:
        ...
        if self.config.use_temperature_variance:
            ...
            if variance_score > 0:
                enhanced_score = (
                    1 - self.config.variance_weight
                ) * enhanced_score + self.config.variance_weight * variance_score
...
class APIEnhancedEntailmentCoherence(EntailmentCoherence):
    ...
    def compute(self, prop_set: PropositionSet) -> CoherenceResult:
        ...
        # Combine results
        enhanced_score = self._combine_entailment_scores(
            base_result.score, api_enhancement
        )
    ...
    def _compute_api_entailment_enhancement(
        self, prop_set: PropositionSet
    ) -> Dict[str, Any]:
            ...
            for i in range(len(propositions)):
                for j in range(i + 1, len(propositions)):
                    entailment_result = self.provider.classify_entailment(
                        premise=propositions[i], hypothesis=propositions[j]
                    )
        ...
        return {
            ...
            "average_entailment": (
                np.mean(entailment_scores) if entailment_scores else 0.5
            ),
        }
    ...
    def _combine_entailment_scores(
        self, base_score: float, api_enhancement: Dict[str, Any]
    ) -> float:
