...
class EmbeddingCache:
    ...
    def _evict_old_entries(self):
        ...
        num_to_remove = (
            len(self._memory_cache) - self.max_size + 100
...
def cached_computation(
    cache: Optional[ComputationCache] = None, ttl_seconds: int = 3600
):
    """
...
class CachedEncoder:
    ...
    def __init__(self, encoder, cache: Optional[EmbeddingCache] = None):
        ...
        self.model_name = getattr(
            encoder, "model_name", getattr(encoder.__class__, "__name__", "unknown")
        )
    ...
    def encode(
        ...
        if texts_to_encode:
            ...
            for i, (text, embedding) in enumerate(
                zip(texts_to_encode, fresh_embeddings)
            ):
