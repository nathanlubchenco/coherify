...
class BatchProcessor:
        ...
        if uncached_requests:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for req in uncached_requests:
                    ...
                    future = executor.submit(self._process_single_request, req)
                ...
                for future in as_completed(future_to_request):
                    ...
                    try:
                        ...
                        results.append(
                            BatchResult(
                                request_id=req.id, response=f"Error: {e}", latency=0.0
                            )
                        )
    ...
    def process_dataset(
        self, prompts: List[str], params: Dict[str, Any] = None
    ) -> List[str]:
        """
        ...
        requests = [
            BatchRequest(id=f"req_{i}", prompt=prompt, params=params)
        ...
        for i in range(0, len(requests), self.batch_size):
            ...
            print(
                f"📦 Processing batch {i//self.batch_size + 1}/{len(requests)//self.batch_size + 1}..."
            )
    ...
    def print_stats(self):
        ...
        if self.cache and cache_stats["hits"] > 0:
            saved_cost = cache_stats["hits"] * (
                self.total_cost / max(1, cache_stats["misses"])
            )
...
class SmartBatchScheduler:
    ...
    def schedule_prompts(
        ...
        for group in groups:
            sorted_group = sorted(
                group, key=lambda p: priorities[prompts.index(p)], reverse=True
            )
