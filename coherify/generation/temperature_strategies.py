    ...
    def sample(self, k: int) -> List[float]:
        ...
        if self.distribution == "uniform":
            ...
            log_temps = np.linspace(
                np.log(max(0.01, self.range_min)), 
                np.log(self.range_max), 
                k
            )
            return list(np.exp(log_temps))
        elif self.distribution == "gaussian":
            # Centered around base_temp with variation
            return list(np.random.normal(self.base_temp, 0.2, k).clip(self.range_min, self.range_max))
        else:
            # Default to uniform
            return list(np.linspace(self.range_min, self.range_max, k))
...
class AdaptiveTemperatureSelector:
    ...
    def classify_question(self, question: str) -> str:
        ...
        # Factual indicators
        factual_keywords = [
            "what is", "when did", "who is", "where is", "how many",
            "define", "list", "name", "which", "calculate", "true or false"
        ]
        ...
        # Creative indicators
        creative_keywords = [
            "imagine", "create", "design", "story", "poem", "creative",
            "what if", "suppose", "hypothetical", "fiction"
        ]
        ...
        exploratory_keywords = [
            "explore", "discuss", "analyze", "compare", "contrast",
            "opinion", "perspective", "elaborate", "explain why"
        ]
    ...
    def select_temperatures(self, 
                           question: str, 
                           k: int = 5,
                           difficulty: Optional[str] = None) -> List[float]:
        """
    ...
    def get_temperature_schedule(self, 
                                 questions: List[str],
...
def get_optimal_temperatures(question: str, 
                            k: int = 5,
