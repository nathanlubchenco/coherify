...
from coherify.generation.temperature_strategies import get_optimal_temperatures
from coherify.providers import AnthropicProvider, OpenAIProvider
...
class ModelRunner:
    ...
    def _setup_provider(self, config: Dict[str, Any]):
        ...
        if provider_name == "openai":
            ...
            return OpenAIProvider(
                api_key=api_key, model_name=config.get("model", "gpt-4o-mini")
            )
            ...
            if not api_key:
                print(
                    "⚠️  No Anthropic API key found. Set ANTHROPIC_API_KEY environment variable."
                )
                return None
    ...
    def generate_k_responses(
        self, prompt: str, k: int = 5, **kwargs
        ...
        if "temperatures" not in kwargs:
            ...
            )  # Pop to avoid passing to generate_response
            temperatures = get_optimal_temperatures(prompt, k, strategy)
        else:
            temperatures = kwargs.pop(
                "temperatures", None
            )  # Pop to avoid passing to generate_response

        # Remove any other custom kwargs that shouldn't go to generate_response
        kwargs_copy = kwargs.copy()

        for i in range(k):
            temp = temperatures[i % len(temperatures)]
            response = self.generate_response(prompt, temperature=temp, **kwargs_copy)
            responses.append(response)

        return responses

    def generate_for_benchmark(
        self, samples: List[Dict[str, Any]], question_key: str = "question"
    ) -> List[str]:
        """
