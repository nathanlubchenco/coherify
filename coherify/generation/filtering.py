...
class CoherenceFilter:
    ...
    def filter_candidates(self,
                         context: str,
                         candidates: List[str],
                         baseline_coherence: Optional[float] = None) -> FilterResult:
        """
    ...
    def _compute_statistics(self,
        ...
        stats = {
            ...
            "coherence_statistics": {
                ...
                "passed": self._score_stats(passed_coherence) if passed_coherence else {},
                "filtered": self._score_stats(filtered_coherence) if filtered_coherence else {}
            }
        }
...
class AdaptiveCoherenceFilter(CoherenceFilter):
    ...
    def _adapt_threshold(self, result: FilterResult):
        ...
        pass_rate = len(result.passed_candidates) / max(result.metadata["total_candidates"], 1)
...
class MultiStageFilter:
    ...
    def __init__(self,
        ...
        if stages is None:
            stages = [
                {"name": "basic_filter", "threshold": 0.2, "description": "Basic coherence filter"},
                {"name": "quality_filter", "threshold": 0.4, "description": "Quality coherence filter"},
                {"name": "excellence_filter", "threshold": 0.6, "description": "Excellence coherence filter"}
            ]
    ...
    def filter_candidates(self,
                         context: str,
                         candidates: List[str],
                         target_stage: Optional[str] = None) -> Dict[str, FilterResult]:
        """
        ...
        for stage in self.stages:
            ...
            self.stage_statistics[stage_name].append({
                ...
                "filter_rate": 1 - (len(stage_result.passed_candidates) / max(len(current_candidates), 1)),
                "avg_coherence": np.mean(stage_result.coherence_scores) if stage_result.coherence_scores else 0
            })
        ...
        for stage_name, stage_data in self.stage_statistics.items():
            if stage_data:
                pipeline_stats[stage_name] = {
                    ...
                    "avg_output_count": np.mean([d["output_count"] for d in stage_data]),
    ...
    def recommend_stage(self, 
                       context: str,
                       sample_candidates: List[str],
                       target_output_count: int = 5) -> str:
        """
...
class ContextAwareFilter(CoherenceFilter):
    ...
    def analyze_context(self, context: str) -> str:
        ...
        if any(word in context_lower for word in ["technical", "algorithm", "method", "process"]):
            return "technical"
        elif any(word in context_lower for word in ["story", "creative", "imagine", "describe"]):
            return "creative"
        elif any(word in context_lower for word in ["fact", "evidence", "research", "study"]):
            return "factual"
        elif any(word in context_lower for word in ["chat", "talk", "conversation", "discuss"]):
            return "conversational"
        else:
            return "general"
    
    def filter_candidates(self, context: str, candidates: List[str], **kwargs) -> FilterResult:
